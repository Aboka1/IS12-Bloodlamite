...
/obj/item/stack/medical/ointment
	name = "ointment"
	desc = "Used to treat the burns you got from taking the flame trooper head on."
	gender = PLURAL
...
/obj/item/stack/medical/ointment/attack(mob/living/carbon/M as mob, mob/living/carbon/human/user as mob)
	if(..())
		return 1
	...
	if (istype(M, /mob/living/carbon/human))
		...
		if(affecting.is_salved())
			...
			if(!do_mob(user, M, backwards_skill_scale(user.SKILL_LEVEL(medical)) * 5))
...
/obj/item/stack/medical/advanced/ointment/attack(mob/living/carbon/M as mob, mob/living/carbon/human/user as mob)
	if(..())
		return 1
...
/obj/item/stack/medical/splint
	...
	max_amount = 10
	animal_heal = 0
	var/list/splintable_organs = list(BP_HEAD, BP_NECK, BP_CHEST, BP_L_ARM, BP_R_ARM, BP_L_LEG, BP_R_LEG, BP_L_HAND, BP_R_HAND, BP_L_FOOT, BP_R_FOOT)	//List of organs you can splint, natch.

/obj/item/stack/medical/splint/attack(mob/living/carbon/M as mob, mob/living/carbon/human/user as mob)
	if(..())
		return 1
	...
	if (istype(M, /mob/living/carbon/human))
		...
		if(do_after(user, backwards_skill_scale(user.SKILL_LEVEL(medical)) * 5, M))
			if(M == user && prob(75))
