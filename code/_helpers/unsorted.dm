...
//this should recursively check the type of loc until it reaches a turf or nullspace - Lohi
/proc/is_parent_loc(atom/to_check, target_type)
	var/atom/target = to_check
	while(isloc(target) && !isturf(target))
		target = target.loc
